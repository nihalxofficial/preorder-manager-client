...
import { getPreorderById } from "@/lib/api/preorders";
import { updatePreorderAction } from "@/lib/actions/preorders";

export default function EditPreorderPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await updatePreorderAction(id, formDataObj);
      if (result.success) {
        router.push("/preorders");
        router.refresh();
      } else {
        console.error("Error updating preorder:", result.error);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50/30 to-white p-4 md:p-8">
      <div className="container mx-auto max-w-3xl">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl shadow-sky-100/40 border border-sky-100/40 p-6 md:p-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Edit Preorder</h1>
